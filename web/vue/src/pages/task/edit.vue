    ...
    <el-main>
      <el-form ref="form" :model="form" :rules="formRules" label-width="180px">
        ...
        <el-row v-if="form.level === 1">
          <el-col>
            <el-alert title="主任务可以配置多个子任务, 当主任务执行完成后，自动执行子任务" type="info" :closable="false">
            </el-alert>
            <el-alert title="任务类型新增后不能变更" type="info" :closable="false">
            </el-alert>
            <el-alert title="强依赖: 主任务执行成功，才会运行子任务" type="info" :closable="false">
            </el-alert>
            <el-alert title="弱依赖: 无论主任务执行是否成功，都会运行子任务" type="info" :closable="false" />
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="7">
          ...
          <el-col :span="7" v-if="form.level === 1">
            <el-form-item label="依赖关系">
              <el-select v-model.trim="form.dependency_status">
                <el-option v-for="item in dependencyStatusList" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="8" v-if="form.protocol === 1 ">
            <el-form-item label="请求方法">
              <el-select key="http-method" v-model.trim="form.http_method">
                <el-option v-for="item in httpMethods" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8" v-else>
            <el-form-item label="任务节点">
              <el-select key="shell" v-model="selectedHosts" filterable multiple placeholder="请选择">
                <el-option v-for="item in hosts" :key="item.id" :label="item.alias + ' - ' + item.name" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col>
            <el-alert title="命令行参数：竖线分隔；例如：a|b|c,表示第1、2、3个参数默认分别为a,b,c" type="info" :closable="false" />
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="16">
            <el-form-item label="命令参数" prop="command_args">
              <el-input type="input" size="medium" width="100" placeholder="请输入命令行参数" v-model="form.command_args">
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="16">
            <el-form-item label="命令" prop="command">
              <el-input type="textarea" :rows="5" size="medium" width="100" :placeholder="commandPlaceholder" v-model="form.command">
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col>
            <el-alert title="任务执行超时强制结束, 取值0-86400(秒), 默认0, 不限制" type="info" :closable="false">
            </el-alert>
            <el-alert title="单实例运行, 前次任务未执行完成，下次任务调度时间到了是否要执行, 即是否允许多进程执行同一任务" type="info" :closable="false">
            </el-alert> <br>
          </el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="任务通知">
              <el-select v-model.trim="form.notify_status">
                <el-option v-for="item in notifyStatusList" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8" v-if="form.notify_status !== 1">
            <el-form-item label="通知类型">
              <el-select v-model.trim="form.notify_type">
                <el-option v-for="item in notifyTypes" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="8" v-if="form.notify_status !== 1 && form.notify_type === 3">
            <el-form-item label="发送Channel">
              <el-select key="notify-slack" v-model="selectedSlackNotifyIds" filterable multiple placeholder="请选择">
                <el-option v-for="item in slackChannels" :key="item.id" :label="item.name" selected="true" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="16">
            <el-form-item label="备注">
              <el-input type="textarea" :rows="3" size="medium" width="100" v-model="form.remark">
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  created () {
    ...
    taskService.detail(id, (taskData, hosts) => {
      ...
      this.form.command_args = taskData.command_args
